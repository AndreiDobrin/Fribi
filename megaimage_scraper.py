            ...
            for category_link in links:
                ...
                for item in items:
                    ...
                    for product in products:
                        ...
                        link = item.find(attrs={"data-testid": "product-block-image-link"})
                        driver.get("https://www.mega-image.ro" + link['href'])
                        ...
                        if product[4] == name and product[8] == brand: # produsul a fost gasit. se verifica daca detaliile s-au schimbat
                            ...
                            '''
                            item_in_list_form = ["id","id_category", price, promo , name, description, ingredients, image_src, brand, price_per_unit, unit]
                            print(item_in_list_form[2:])

                            if float(product[2]) != float(price) or product[3] != promo or product[4] != name or product[5] != description or product[6] != ingredients or product[7] != image_src or product[8] != brand or float(product[9]) != float(price_per_unit) or product[10] != unit:
                                
                                if float(product[2]) != float(price):
                                    print(f"Pretul difera: {float(product[2])} vs {float(price)}")
                                if product[3] != promo:
                                    print(f"Promotia difera: {product[3]} vs {promo}")
                                if product[4] != name:
                                ...
                                cursor = connection.cursor()
                                sql = "UPDATE product SET price = %s, offer = %s, product_name = %s, description = %s, ingredients = %s, image_src = %s, product_brand = %s, price_per_unit = %s, unit = %s WHERE product_name = %s AND product_brand = %s"
                                val = (price, promo, name, description, ingredients, image_src, brand, price_per_unit, unit, name, brand)
                                print(val)
